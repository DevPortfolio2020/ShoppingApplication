...
namespace ShoppingConsole
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            foreach (var product in products)
            {
                ...
                do
                {
                    try
                    {
                        ...
                        bool negative = order.Quantity < 0;
                        if (!negative)
                        {
                            validEntry = true;
                        }
                ...
                if (order.Quantity > 0)
                {
                    ...
                    {
                        order.DiscountPerLineItem = 0.10m;

                    }

                    else if (order.Quantity >= 25 && order.Quantity < 50)
                    {
                        order.DiscountPerLineItem = 0.25m;

                    }

                    else if (order.Quantity >= 50)
                    {
                        order.DiscountPerLineItem = 0.50m;
            ...
            //Print the totals
            var totalPerLineItem = new List<decimal>();
            var pricePerLineItem = new List<decimal>();

            foreach (var order in productsOrdered)


            {
                Console.WriteLine("Note: Orders must include at least one item to be shown below.");
